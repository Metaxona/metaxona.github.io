<script>
    ...
    const {postsByCategory, category} = data

    let pageMetadata = {
        title: `Metaxona - Blog Categories: ${category}`,
        keywords: `website, blog, categories, ${category}`,
        ...
        audience: "Everyone",
        type: "Website",
        url: `https://metaxona.com/blog/categories/${encodeURIComponent(category)}`,
...
<div class="p-4">
    <div class="font-bold text-4xl text-center mb-5">{category}</div>
    <p class="text-sm capitalize mt-5 pb-20 text-center"><a href="/blog">Blog</a> / <a href="/blog/categories">Categories</a> / <a href="/blog/categories/{encodeURIComponent(category)}" aria-disabled="true">{category}</a></p>


    <div class="flex flex-row flex-wrap gap-8 justify-center">
        {#each postsByCategory as post}
